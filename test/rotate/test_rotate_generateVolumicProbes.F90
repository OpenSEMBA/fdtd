...
integer function test_rotate_generate_volumic_probes() bind(C) result(err)
    ...
    ! Test case 1: Rotate VolProbes in mpidir=2 direction
    mpidir = 2
    call setup_volumic_probe_test(this, 2)
    
    ! Set up test data
    call init_volumic_probe_data(this, 1, 1, 4, 2, 5, 3, 6, 1, 1, 1, 1, "probe1")
    call init_volumic_probe_data(this, 2, 5, 8, 6, 9, 7, 10, 1, 1, 1, 2, "probe2")
    
    ! Call rotation and verify results
    call rotate_generateVolumicProbes(this, mpidir)
    call verify_volumic_probe_rotation_y(test_err, this)
    
    ! Clean up
    call cleanup_volumic_probe_test(this)
    
    ! Test case 2: Rotate VolProbes in mpidir=1 direction
    mpidir = 1
    call setup_volumic_probe_test(this, 2)
    ...
    ! Call rotation and verify results
    call rotate_generateVolumicProbes(this, mpidir)
    call verify_volumic_probe_rotation_x(test_err, this)
    
    ! Clean up
    call cleanup_volumic_probe_test(this)
    
    ! Test case 3: Verify behavior with no VolProbes
    mpidir = 2
    call setup_volumic_probe_test(this, 0)
    call rotate_generateVolumicProbes(this, mpidir)
    call expect_eq_int(test_err, 0, this%VolPrb%length, "rotate_generateVolumicProbes: length should remain 0")
    call cleanup_volumic_probe_test(this)
...
subroutine setup_volumic_probe_test(this, n_probes)
    ...
    this%VolPrb%len_cor_max = 1  ! Each probe has one coordinate set
    if (n_probes > 0) then
...
subroutine verify_volumic_probe_rotation_y(test_err, this)
    integer, intent(inout) :: test_err
    type(Parseador), intent(in) :: this
    
    ! Verify VolumicProbe 1 (rotated around y-axis)
    call expect_eq_int(test_err, -3, this%VolPrb%collection(1)%cordinates(1)%Xi, "rotate_generateVolumicProbes: Xi(1) should be rotated")
    call expect_eq_int(test_err, -6, this%VolPrb%collection(1)%cordinates(1)%Xe, "rotate_generateVolumicProbes: Xe(1) should be rotated")
    ...
    call expect_eq_int(test_err, -1, this%VolPrb%collection(1)%cordinates(1)%Zi, "rotate_generateVolumicProbes: Zi(1) should be rotated")
    call expect_eq_int(test_err, -4, this%VolPrb%collection(1)%cordinates(1)%Ze, "rotate_generateVolumicProbes: Ze(1) should be rotated")
    ...
    call expect_eq_int(test_err, 1, this%VolPrb%collection(1)%cordinates(1)%Ytrancos, "rotate_generateVolumicProbes: Ytrancos(1) should remain unchanged")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(1)%cordinates(1)%Ztrancos, "rotate_generateVolumicProbes: Ztrancos(1) should remain unchanged")
    ...
    ! Verify VolumicProbe 2 (rotated around y-axis)
    call expect_eq_int(test_err, -7, this%VolPrb%collection(2)%cordinates(1)%Xi, "rotate_generateVolumicProbes: Xi(2) should be rotated")
    call expect_eq_int(test_err, -10, this%VolPrb%collection(2)%cordinates(1)%Xe, "rotate_generateVolumicProbes: Xe(2) should be rotated")
    ...
    call expect_eq_int(test_err, -5, this%VolPrb%collection(2)%cordinates(1)%Zi, "rotate_generateVolumicProbes: Zi(2) should be rotated")
    call expect_eq_int(test_err, -8, this%VolPrb%collection(2)%cordinates(1)%Ze, "rotate_generateVolumicProbes: Ze(2) should be rotated")
    ...
    call expect_eq_int(test_err, 1, this%VolPrb%collection(2)%cordinates(1)%Ytrancos, "rotate_generateVolumicProbes: Ytrancos(2) should remain unchanged")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(2)%cordinates(1)%Ztrancos, "rotate_generateVolumicProbes: Ztrancos(2) should remain unchanged")
...
end subroutine verify_volumic_probe_rotation_y

subroutine verify_volumic_probe_rotation_x(test_err, this)
    integer, intent(inout) :: test_err
    type(Parseador), intent(in) :: this
    
    ! Verify VolumicProbe 1 (rotated around x-axis)
    ...
    call expect_eq_int(test_err, -3, this%VolPrb%collection(1)%cordinates(1)%Yi, "rotate_generateVolumicProbes: Yi(1) should be rotated")
    call expect_eq_int(test_err, -6, this%VolPrb%collection(1)%cordinates(1)%Ye, "rotate_generateVolumicProbes: Ye(1) should be rotated")
    call expect_eq_int(test_err, 2, this%VolPrb%collection(1)%cordinates(1)%Zi, "rotate_generateVolumicProbes: Zi(1) should be rotated")
    call expect_eq_int(test_err, 5, this%VolPrb%collection(1)%cordinates(1)%Ze, "rotate_generateVolumicProbes: Ze(1) should be rotated")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(1)%cordinates(1)%Xtrancos, "rotate_generateVolumicProbes: Xtrancos(1) should remain unchanged")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(1)%cordinates(1)%Ytrancos, "rotate_generateVolumicProbes: Ytrancos(1) should remain unchanged")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(1)%cordinates(1)%Ztrancos, "rotate_generateVolumicProbes: Ztrancos(1) should remain unchanged")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(1)%cordinates(1)%Or, "rotate_generateVolumicProbes: Or(1) should remain unchanged")
    
    ! Verify VolumicProbe 2 (rotated around x-axis)
    ...
    call expect_eq_int(test_err, -7, this%VolPrb%collection(2)%cordinates(1)%Yi, "rotate_generateVolumicProbes: Yi(2) should be rotated")
    call expect_eq_int(test_err, -10, this%VolPrb%collection(2)%cordinates(1)%Ye, "rotate_generateVolumicProbes: Ye(2) should be rotated")
    call expect_eq_int(test_err, 6, this%VolPrb%collection(2)%cordinates(1)%Zi, "rotate_generateVolumicProbes: Zi(2) should be rotated")
    call expect_eq_int(test_err, 9, this%VolPrb%collection(2)%cordinates(1)%Ze, "rotate_generateVolumicProbes: Ze(2) should be rotated")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(2)%cordinates(1)%Xtrancos, "rotate_generateVolumicProbes: Xtrancos(2) should remain unchanged")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(2)%cordinates(1)%Ytrancos, "rotate_generateVolumicProbes: Ytrancos(2) should remain unchanged")
    call expect_eq_int(test_err, 1, this%VolPrb%collection(2)%cordinates(1)%Ztrancos, "rotate_generateVolumicProbes: Ztrancos(2) should remain unchanged")
    call expect_eq_int(test_err, 2, this%VolPrb%collection(2)%cordinates(1)%Or, "rotate_generateVolumicProbes: Or(2) should remain unchanged")
end subroutine verify_volumic_probe_rotation_x
