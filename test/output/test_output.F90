...
integer function test_update_point_probe() bind(c) result(err)
   ...
   call update_outputs(media, materialsPtr, sinpml, control, sgg%tiempo, 1_SINGLE, fields, bounds)
   ...
   call update_outputs(media, materialsPtr, sinpml, control, sgg%tiempo, 2_SINGLE, fields, bounds)
...
integer function test_init_movie_probe() bind(c) result(err)
   ...
   test_err = test_err + assert_integer_equal(outputs(1)%movieProbe%nMeasuredElements, &
...
integer function test_update_movie_probe() bind(c) result(err)
   ...
   call update_outputs(mediaPtr, simulationMaterialsPtr, sinpml_fullsizePtr, &
                       dummyControl, dummysgg%tiempo, 1_SINGLE, fields, dummyBound)
...
integer function test_flush_movie_probe() bind(c) result(err)
   ...
   expectedPath = trim(adjustl(outputs(1)%movieProbe%path))//'_ts0001.vtu'
   test_err = test_err + assert_file_exists(expectedPath)

   expectedPath = trim(adjustl(outputs(1)%movieProbe%path))//'_ts0002.vtu'
   test_err = test_err + assert_file_exists(expectedPath)
...
integer function test_init_frequency_slice_probe() bind(c) result(err)
   ...
   test_err = test_err + assert_integer_equal(outputs(1)%frequencySliceProbe%nMeasuredElements, &
      expectedNumMeasurments, 'Unexpected number of measurements')
...
integer function test_update_frequency_slice_probe() bind(c) result(err)
   ...
   test_err = test_err + assert_complex_equal(outputs(1)%frequencySliceProbe%yValueForFreq(3,5), &
      (0.2_CKIND, 0.2_CKIND), 1e-5_RKIND, 'Value error')
