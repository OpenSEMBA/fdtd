integer function test_init_solver() bind (C) result(err)
   ...
   media = initMedia(semba%sgg,semba%sggMiNo,semba%sggMiEx,semba%sggMiEy,semba%sggMiEz,semba%sggMiHx,semba%sggMiHy,semba%sggMiHz,semba%sggMtag)

   call solver%init_control(semba%l, semba%maxSourceValue, semba%time_desdelanzamiento)
   call solver%init(semba%sgg,semba%eps0, semba%mu0, media, semba%SINPML_fullsize, semba%fullsize, semba%tag_numbers)
