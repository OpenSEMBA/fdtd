integer function test_output_transport_serial() bind(c) result(err)
   ! Verifies the serial identity path used by root-aggregated flushes.
   ...
   real(real64), allocatable :: gathered_batch(:)
   ...
   call transfer_flush_batch(transport, [3.0_real64, 4.0_real64], gathered_batch, counts, displacements, status)
   err = err + assert_integer_equal(status, OUTPUT_TRANSPORT_SUCCESS, 'Serial batch transfer failed')
   err = err + assert_true(all(gathered_batch == [3.0_real64, 4.0_real64]), 'Serial batch values changed')
