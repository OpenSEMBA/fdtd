...
def test_paul_9_6(tmp_path):
    ...
    p_expected = [
        Probe(OUTPUTS_FOLDER + "paul_9_6.fdtd_start_voltage_two_wires_V_5_5_1.dat"),
        Probe(OUTPUTS_FOLDER + "paul_9_6.fdtd_end_voltage_two_wires_V_5_5_795.dat"),
    ]

    probe_voltage_left = solver.getSolvedProbeFilenames("start_voltage")[0]
    probe_voltage_right = solver.getSolvedProbeFilenames("end_voltage")[0]
...
def test_spice_connectors_diode(tmp_path):
    ...
    p_expected = [
        Probe(
            OUTPUTS_FOLDER + "spice_connectors.fdtd_start_voltage_wire_V_10_10_8.dat"
        ),
        Probe(OUTPUTS_FOLDER + "spice_connectors.fdtd_end_voltage_wire_V_10_10_12.dat"),
    ]

    probe_voltage_left = solver.getSolvedProbeFilenames("start_voltage_wire")[0]
    probe_voltage_right = solver.getSolvedProbeFilenames("end_voltage_wire")[0]
    probe_files = [probe_voltage_left, probe_voltage_right]
...
def test_line_multiline_junction(tmp_path):
    ...
    p_expected = [
        Probe(OUTPUTS_FOLDER + "line_multiline_junction.fdtd_s4_end_s4_V_5_5_159.dat"),
        Probe(OUTPUTS_FOLDER + "line_multiline_junction.fdtd_s5_end_s5_V_5_5_159.dat"),
...
def test_spice_opamp_saturation(tmp_path):
    ...
    p_solved = Probe(solver.getSolvedProbeFilenames("opamp_voltage_wire1")[0])
...
def test_spice_zener(tmp_path):
    ...
    p_expected = Probe(OUTPUTS_FOLDER + "zener.fdtd_end_voltage_wire_V_10_10_12.dat")
    t_exp = p_expected.data["time"].to_numpy()[:-1]
    v_exp = p_expected.data["voltage_0"].to_numpy()[:-1]

    p_solved = Probe(solver.getSolvedProbeFilenames("end_voltage_")[0])
    t_sol = p_solved.data["time"].to_numpy()[:-1]
    v_sol = p_solved.data["voltage_0"].to_numpy()[:-1]
