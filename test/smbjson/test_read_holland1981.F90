...
integer function test_read_holland1981_named_wire() bind (C) result(err)
   use smbjson_m
   use smbjson_testingTools

   implicit none

   character(len=*),parameter :: filename = PATH_TO_TEST_DATA//INPUT_EXAMPLES//'holland1981_named_wire.fdtd.json'
   type(Parseador_t) :: problem, expected
   type(parser_t) :: parser
   err = 0
...
contains
   function expectedProblemDescription() result (ex)
      ...
      ex%Sonda%collection(1)%len_cor = 1
      ex%Sonda%collection(1)%cordinates(1)%tag = "mid_point"
      ex%Sonda%collection(1)%cordinates(1)%Xi = 6
      ex%Sonda%collection(1)%cordinates(1)%Yi = 0
      ex%Sonda%collection(1)%cordinates(1)%Zi = 0
      ex%Sonda%collection(1)%cordinates(1)%Or = NP_COR_WIRECURRENT
      
      ! Expected thin wires with named material and element (issue #304).
      ! Material name "steel_wire" and element name "wire_segment" must be preserved.
      allocate(ex%tWires%tw(1))
      ex%tWires%tw(1)%rad=0.02
      ex%tWires%tw(1)%dispfile = trim(adjustl(" "))
      ex%tWires%tw(1)%dispfile_LeftEnd = trim(adjustl(" "))
      ...
      ex%tWires%tw(1)%twc(1:10)%tag = "steel_wire@wire_segment"
