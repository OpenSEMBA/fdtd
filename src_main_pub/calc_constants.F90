        ...
        do r=0,sgg%NumMedia
         ...
         if  ((sgg%Med(R)%Is%already_YEEadvanced_byconformal).or.(sgg%Med(R)%Is%split_and_useless)) then
            ...
            if (sgg%Med(R)%Is%ConformalPEC) then
               if (sgg%Med(R)%Is%Volume) then 
                  g%g1(r)  = 1 
                  g%g2(r)  = sgg%dt /Epsilon
                  g%gm1(r) = 1
                  g%gm2(r) = sgg%dt/ Mu
               else if (sgg%Med(R)%Is%Surface)
                  g%g1(r)  = 1.0_RKIND
                  g%g2(r)  = 0.0_RKIND
                  g%gm1(r) = 1.0_RKIND
                  g%gm2(r) = 0.0_RKIND
...
!!!!! hasta aqui lo comentado a 090519. Los calculos de g%gm1 y g%gm2 no los hace InitSGBCs y sus incertidumbres son nulas. asi que los hago aqui                       
            ...
            elseif  ((sgg%Med(R)%Is%EDispersive).and.(.not.sgg%Med(R)%Is%MDispersive).and.(.not.sgg%Med(r)%Is%EdispersiveANIS)) then
               ...
               g%gm2(r)=sgg%dt / Mu                   / (1+ SigmaM*sgg%dt/(2.0_RKIND *  Mu ))
               if (g%gm1(r) < 0.0_RKIND) then !exponential time stepping
                  g%gm1(r)=exp(- Sigmam * sgg%dt / (Mu ))
                  g%gm2(r)=(1.0_RKIND-g%gm1(r))/ Sigmam
               endif
...
!!call AdvanceelectricCPML          (sgg%NumMedia, b       ,sggMiEx,sggMiEy,sggMiEz,g%g2,Ex,Ey,Ez,Hx,Hy,Hz)
...
!!call AdvancePlaneWaveE(sgg,n, b       ,g%g2,Idxh,Idyh,Idzh,Ex,Ey,Ez)
!!call AdvanceNodalE(sgg,sggMiEx,sggMiEy,sggMiEz,sgg%NumMedia,n, b,g%g2,Idxh,Idyh,Idzh,Ex,Ey,Ez)
!!
!!call AdvanceAnisotropicH(sgg%alloc,ex,ey,ez,hx,hy,hz,Idxe,Idye,Idze,Idxh,Idyh,Idzh)
!!call Advance_Hx           (Hx, Ey, Ez, Idye, Idze, sggMiHx, b,g%gm1,g%gm2)
!!call Advance_Hy           (Hy, Ez, Ex, Idze, Idxe, sggMiHy, b,g%gm1,g%gm2)
!!call Advance_Hz           (Hz, Ex, Ey, Idxe, Idye, sggMiHz, b,g%gm1,g%gm2)
