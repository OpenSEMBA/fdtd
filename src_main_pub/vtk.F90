...
!parte imaginaria
                           ...
                           if ((.not.dontwritevtk).or.(sgg%observation(ii)%P(1)%What==mapvtk)) then !el mapvtk lo procesa siempre
                               ...
                               if (freqDomain) then
                                   ...
                                   do i_sub_time=0,total_sub_times
                                       ...
                                       fichero_input=fichero(1:iroot1-1)//'_n_'//trim(adjustl(char_i_sub_time))//'.vtk' 
 ...
 !!                          DEALLOCATE (NewSerialized%ValorComplex)
   ...
   SUBROUTINE write_VTKfile(sgg,fichero,iroot2, Serialized,  numberOfSerialized,Nodes,Numnodes,Elems,NumEdges,NumQuads,time,  &
                            i_sub_time,total_sub_times,FreqDomain,what,sggMtag)
...
!  Vectorial 0124
         ...
         do conta=1,numberOfSerialized                                     
              ...
              phase_y=atan2(AIMAG(Serialized%valorComplex_x(1,conta)),REAL(Serialized%valorComplex_y(1,conta)))
              phase_z=atan2(AIMAG(Serialized%valorComplex_x(1,conta)),REAL(Serialized%valorComplex_z(1,conta)))
