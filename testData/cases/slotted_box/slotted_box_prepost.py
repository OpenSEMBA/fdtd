...
runs the existing DMMA FDTD case and obtains the simulated electric shielding
effectiveness from the total and incident electric fields at the cavity centre.
The FDTD model uses a 45 degree, p-polarized plane wave and a 10 mm Yee-cell
size; the Robinson reference itself assumes normal incidence.
...
# %% DMMA FDTD simulation and SEee calculation
def simulated_shielding_effectiveness():
    """Run the case and return FDTD SEee from vector total/incident fields."""
    solver = FDTD(input_filename=INPUT_FILE, path_to_exe=SEMBA_EXE)
    solver.cleanUp()
    solver.run()

    probes = {}
    for filename in solver.getSolvedProbeFilenames("Point probe"):
    ...
    total_spectrum = np.fft.rfft(field, axis=1)
    incident_spectrum = np.fft.rfft(incident, axis=1)

    with np.errstate(divide="ignore", invalid="ignore"):
        se_electric = 20.0 * np.log10(
    ...
    valid = (
        (frequency >= 10.0e6)
        & (frequency <= 3.0e9)
        & np.isfinite(se_electric)
    )
    return frequency[valid], se_electric[valid]


# %% Simulated-versus-analytical SEee plot
frequency = np.geomspace(10.0e6, 3.0e9, 4096)
se_electric, _ = shielding_effectiveness(frequency)
valid_electric = np.isfinite(se_electric)
frequency_fdtd, se_electric_fdtd = simulated_shielding_effectiveness()
...
plt.axvline(c / (2.0 * A), color="0.4", linestyle=":",
            label=r"TE$_{10}$ cutoff")
