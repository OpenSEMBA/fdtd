...
InitialLumpedBulk_probe = Probe(solver_lumped.getSolvedProbeFilenames("Initial current")[0])
...
V_in_f = fft(V_in)
I_teo_f = V_in_f*(1/100 + 1j*2*np.pi*fq*1e-12)

I_teo = ifft(I_teo_f)

plt.figure()
plt.plot(InitialLumpedBulk_probe['time'].to_numpy(), InitialLumpedBulk_probe['current'].to_numpy(), label='Initial current on bulk', color='blue')
