...
# %% Interpolate excitation at probe times
...
# %% Checks
assert np.corrcoef(exc_interp, probe_LL["current"].to_numpy())[0, 1] > 0.9999
assert np.max(np.abs(probe_LU["current"].to_numpy())) < 2e-3
assert np.max(np.abs(probe_UU["current"].to_numpy())) < 2e-3
