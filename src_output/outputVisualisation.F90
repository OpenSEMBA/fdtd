...
contains
    ...
    subroutine verify_visualisation_artifact(path, artifact, status)
       character(len=*), intent(in) :: path
       type(output_artifact_t), intent(in) :: artifact
       integer, intent(out) :: status

       if ((artifact%kind /= OUTPUT_ARTIFACT_VISUALISATION_METADATA .and. &
            artifact%kind /= OUTPUT_ARTIFACT_VISUALISATION_DATA) .or. &
           .not. output_artifact_identity_is_valid(artifact) .or. .not. file_exists(path)) then
          status = VISUALISATION_IO_ERROR
       else
          status = VISUALISATION_SUCCESS
       end if
    end subroutine verify_visualisation_artifact
